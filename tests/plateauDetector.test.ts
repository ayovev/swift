...
describe("getPlateauInsights — real sample data", () => {
  ...
  it("classifies the real Back Squat decline as plateaued_other, not plateaued_body_comp", async () => {
    const workouts = await loadSampleRows();
    const inbodyScans = await loadSampleInBodyRows();
    const insights = getPlateauInsights(workouts, inbodyScans, AS_OF);
    ...
    // fixture: recent-3 avg (05/14/2025, 01/12/2026, 08/02/2026) = 198.33 lb
    // vs previous-3 avg (08/13/2024, 10/15/2024, 12/13/2024) = 208.33 lb,
    // ~-4.8% — a real decline. Over that same window (2024-08-13 →
    // 2026-08-02), the fixture's body comp is steadily *improving* (SMM
    // 85.0 → 88.5 lb, Fat Mass 26.0 → 20.1 lb), so the decline isn't a body
    // comp story — this is the single most important regression here: the
    // detector must not blame body comp when the data doesn't support it.
    expect(backSquat!.performanceTrend.direction).toBe("down");
    expect(backSquat!.windowStart).toBe("2024-08-13");
...
describe("getPlateauInsights — grouping and normalization", () => {
  it("merges 'Deadlift' and 'Deadlifts' into one subject", () => {
    const workouts = [
      row({ date: "01/01/2024", title: "DEADLIFT", barbell_lift: "Deadlift", score_type: "Load", best_result_raw: "225" }),
      row({ date: "02/01/2024", title: "DEADLIFTS", barbell_lift: "Deadlifts", score_type: "Load", best_result_raw: "235" }),
      row({ date: "03/01/2024", title: "DEADLIFT", barbell_lift: "Deadlift", score_type: "Load", best_result_raw: "245" }),
    ];
    const inbodyScans = [
  ...
  it("folds a freeform title match into an already-known barbell_lift-tagged lift", () => {
    const workouts = [
      row({ date: "01/01/2024", title: "FRONT SQUAT", barbell_lift: "Front Squat", score_type: "Load", best_result_raw: "165" }),
      row({ date: "02/01/2024", title: "Front Squat", barbell_lift: "", score_type: "Load", best_result_raw: "175" }),
      row({ date: "03/01/2024", title: "FRONT SQUAT", barbell_lift: "Front Squat", score_type: "Load", best_result_raw: "185" }),
    ];
    const inbodyScans = [
      inbodyRow({ date: "20240201000000" }),
      inbodyRow({ date: "20240301000000" }),
...
describe("getPlateauInsights — eligibility gate", () => {
  it("returns insufficient_data with fewer than 3 same-status entries, even with plenty of InBody scans", () => {
    const workouts = [
      row({ date: "01/01/2024", title: "SNATCH", barbell_lift: "Snatch", score_type: "Load", best_result_raw: "95" }),
      row({ date: "02/01/2024", title: "SNATCH", barbell_lift: "Snatch", score_type: "Load", best_result_raw: "100" }),
    ];
    const inbodyScans = [
  ...
  it("returns insufficient_data with 3+ entries but fewer than 2 InBody scans in the comparison window", () => {
    const workouts = [
      row({ date: "01/01/2024", title: "SNATCH", barbell_lift: "Snatch", score_type: "Load", best_result_raw: "90" }),
      row({ date: "02/01/2024", title: "SNATCH", barbell_lift: "Snatch", score_type: "Load", best_result_raw: "95" }),
      row({ date: "03/01/2024", title: "SNATCH", barbell_lift: "Snatch", score_type: "Load", best_result_raw: "100" }),
    ];
...
describe("getPlateauInsights — confidence tiering", () => {
  ...
  it("is medium at 6 entries and 3 scans in window", () => {
    const workouts = ["01", "02", "03", "04", "05", "06"].map((m, i) =>
      row({ date: `${m}/01/2024`, title: "SNATCH", barbell_lift: "Snatch", score_type: "Load", best_result_raw: String(90 + i) })
    );
    const inbodyScans = [
      inbodyRow({ date: "20240201000000" }),
      inbodyRow({ date: "20240301000000" }),
...
describe("getPlateauInsights — body composition signal combination", () => {
  const decliningLift: SugarWodRow[] = [
    row({ date: "01/01/2024", title: "OHS", barbell_lift: "Overhead Squat", score_type: "Load", best_result_raw: "100" }),
    row({ date: "02/01/2024", title: "OHS", barbell_lift: "Overhead Squat", score_type: "Load", best_result_raw: "100" }),
    row({ date: "03/01/2024", title: "OHS", barbell_lift: "Overhead Squat", score_type: "Load", best_result_raw: "80" }),
  ];
