...
describe("TabNav — ALL_TABS", () => {
  it("has one entry per Overview, Workouts, GPP domain, modality, Body Comp, Plateaus and Phase Alignment", () => {
    ...
    expect(ALL_TABS.filter((t) => t.group === "Phase Alignment")).toHaveLength(1);
  });
});

describe("TabNav — desktop group pills", () => {
  it("selects Overview, Workouts, Body Comp, Plateaus and Phase Alignment directly, with no menu", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Phase Alignment" }));
    expect(onValueChange).toHaveBeenCalledWith(PHASE_ALIGNMENT_TAB);
  });
...
describe("TabNav — mobile select", () => {
  it("lists every tab, grouped into optgroups for everything but Overview", () => {
    ...
    expect(within(select).getByRole("group", { name: "Phase Alignment" })).toBeInTheDocument();
  });
